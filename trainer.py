...
class Trainer:
    ...
    def get_env_copy(self):
        ...
        """
        if self._env:
            env_copy = cloudpickle.loads(cloudpickle.dumps(self._env))
            env_copy.seed(deterministic.get_seed())
            env_copy.action_space.seed(deterministic.get_seed())
            return env_copy#cloudpickle.loads(cloudpickle.dumps(self._env))
        else:
            return None
