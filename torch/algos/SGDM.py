...
class SGDM(RLAlgorithm):
    ...
    def _get_baselines(self, eps, obs):
    ...
    def _train_once(self, itr, eps):
        ...
        baselines = self._get_baselines(eps, obs)
    ...
    def train(self, trainer):
        ...
        if not self._eval_env:
            self._eval_env = self.trainer.get_env_copy()
        last_return = None
        for _ in trainer.step_epochs():
            for _ in range(self._n_samples):
                ...
                eps = self.trainer.obtain_episodes(trainer.step_itr)
                last_return = self._train_once(trainer.step_itr, eps)
                #log_performance(trainer.step_itr, eps, discount=self._discount, prefix='Training') 
                with tabular.prefix('Training/'):
                    tabular.record('NumEpisodes', len(eps.lengths))
                

                self.trainer.step_itr += 1
